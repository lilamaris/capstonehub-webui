...
export default function LoginForm({ className, action, ...props }: React.ComponentProps<'form'>) {
  const [state, formAction, isPending] = React.useActionState(signinAction, undefined)
        ...
        <form action={formAction} className={cn('grid gap-6', className)} {...props}>
          ...
          <div className="grid gap-6">
            <div className="grid gap-3">
              ...
              {state?.errors?.email && <p className="text-xs text-destructive">{state.errors.email.errors}</p>}
            </div>
            <div className="grid gap-3">
              ...
              {state?.errors?.password && <p className="text-xs text-destructive">{state.errors.password.errors}</p>}
            </div>
